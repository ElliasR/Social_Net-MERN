...
const getPlaceById = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError('Place not found, please try again. ', 500);
    return next(error);
  }
...
const getPlacesByUserId = async (req, res, next) => {
  ...
  if (!userWithPlaces || userWithPlaces.places.length === 0) {
    return next(
      new HttpError('No places found with the provided user id.', 404)
    );
  }
...
const deletePlace = async (req, res, next) => {
  ...
  let place;
  try {
    place = await Place.findById(placeId).populate('creator'); //to get access to a document stored in a different collection, powerful.
  } catch (err) {
    const error = new HttpError(
      'No place found with the provided id, please try again. ',
      500
    );
    return next(error);
  }
